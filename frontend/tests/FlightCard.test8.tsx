...
describe('FlightCard Component Unit Test Suite 8', () => {
  ...
  it('renders flight number and calculated fare correctly - Test 8', () => {
    const handleSelect = vi.fn();
    render(<FlightCard flight={mockFlight} onSelect={handleSelect} />);

    expect(screen.getByText('SN-308')).toBeDefined();
    expect(screen.getByText('$560')).toBeDefined();
  });

  it('triggers onSelect callback when Select Flight button is clicked - Test 8', () => {
    const handleSelect = vi.fn();
    render(<FlightCard flight={mockFlight} onSelect={handleSelect} />);
