...
describe('FlightCard Component Unit Test Suite 6', () => {
  ...
  it('renders flight number and calculated fare correctly - Test 6', () => {
    const handleSelect = vi.fn();
    render(<FlightCard flight={mockFlight} onSelect={handleSelect} />);

    expect(screen.getByText('SN-306')).toBeDefined();
    expect(screen.getByText('$560')).toBeDefined();
  });

  it('triggers onSelect callback when Select Flight button is clicked - Test 6', () => {
    const handleSelect = vi.fn();
    render(<FlightCard flight={mockFlight} onSelect={handleSelect} />);
