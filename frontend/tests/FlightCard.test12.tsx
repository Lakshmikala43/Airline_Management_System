...
describe('FlightCard Component Unit Test Suite 12', () => {
  ...
  it('renders flight number and calculated fare correctly - Test 12', () => {
    const handleSelect = vi.fn();
    render(<FlightCard flight={mockFlight} onSelect={handleSelect} />);

    expect(screen.getByText('SN-312')).toBeDefined();
    expect(screen.getByText('$560')).toBeDefined();
  });

  it('triggers onSelect callback when Select Flight button is clicked - Test 12', () => {
    const handleSelect = vi.fn();
    render(<FlightCard flight={mockFlight} onSelect={handleSelect} />);
