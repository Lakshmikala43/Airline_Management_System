...
class ServiceEdgeCaseTest125 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 125")
    void testGetAirportByIataBoundary125() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 125")
    void testCreateAirportBoundary125() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 125).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
