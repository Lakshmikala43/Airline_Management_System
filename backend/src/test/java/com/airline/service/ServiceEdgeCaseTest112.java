...
class ServiceEdgeCaseTest112 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 112")
    void testGetAirportByIataBoundary112() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 112")
    void testCreateAirportBoundary112() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 112).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
