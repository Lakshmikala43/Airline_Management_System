...
class PromotionServiceTest42 {
    ...
    @Test
    @DisplayName("Verify coupon code validation for percentage discounts - Suite 42")
    void testValidatePercentageCoupon42() {
        Promotion promo = Promotion.builder()
            .id((long) 42)
            .couponCode("SKYNOVA42")
            .description("Special Promo 42")
        ...
        when(promotionRepository.findByCouponCode("SKYNOVA42")).thenReturn(Optional.of(promo));

        var resp = promotionService.validateCoupon("SKYNOVA42", BigDecimal.valueOf(500.00));
        assertNotNull(resp);
