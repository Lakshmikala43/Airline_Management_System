...
class ServiceEdgeCaseTest141 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 141")
    void testGetAirportByIataBoundary141() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 141")
    void testCreateAirportBoundary141() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 141).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
