...
class ServiceEdgeCaseTest138 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 138")
    void testGetAirportByIataBoundary138() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 138")
    void testCreateAirportBoundary138() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 138).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
