...
class ServiceEdgeCaseTest70 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 70")
    void testGetAirportByIataBoundary70() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 70")
    void testCreateAirportBoundary70() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 70).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
