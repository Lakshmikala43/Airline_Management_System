...
class ServiceEdgeCaseTest105 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 105")
    void testGetAirportByIataBoundary105() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 105")
    void testCreateAirportBoundary105() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 105).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
