...
class ServiceEdgeCaseTest2 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 2")
    void testGetAirportByIataBoundary2() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 2")
    void testCreateAirportBoundary2() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 2).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
