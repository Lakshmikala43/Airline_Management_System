...
class ServiceEdgeCaseTest69 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 69")
    void testGetAirportByIataBoundary69() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 69")
    void testCreateAirportBoundary69() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 69).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
