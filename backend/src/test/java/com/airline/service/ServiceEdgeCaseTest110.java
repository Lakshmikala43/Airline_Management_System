...
class ServiceEdgeCaseTest110 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 110")
    void testGetAirportByIataBoundary110() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 110")
    void testCreateAirportBoundary110() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 110).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
