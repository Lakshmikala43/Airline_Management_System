...
class ServiceEdgeCaseTest57 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 57")
    void testGetAirportByIataBoundary57() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 57")
    void testCreateAirportBoundary57() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 57).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
