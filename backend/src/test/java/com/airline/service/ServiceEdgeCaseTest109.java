...
class ServiceEdgeCaseTest109 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 109")
    void testGetAirportByIataBoundary109() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 109")
    void testCreateAirportBoundary109() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 109).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
