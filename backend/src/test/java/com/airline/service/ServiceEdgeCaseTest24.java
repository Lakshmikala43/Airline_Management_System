...
class ServiceEdgeCaseTest24 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 24")
    void testGetAirportByIataBoundary24() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 24")
    void testCreateAirportBoundary24() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 24).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
