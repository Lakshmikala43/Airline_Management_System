...
class ServiceEdgeCaseTest124 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 124")
    void testGetAirportByIataBoundary124() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 124")
    void testCreateAirportBoundary124() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 124).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
