...
class ServiceEdgeCaseTest111 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 111")
    void testGetAirportByIataBoundary111() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 111")
    void testCreateAirportBoundary111() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 111).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
