...
class ServiceEdgeCaseTest1 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 1")
    void testGetAirportByIataBoundary1() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 1")
    void testCreateAirportBoundary1() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 1).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
