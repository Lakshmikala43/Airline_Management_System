...
class ServiceEdgeCaseTest7 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 7")
    void testGetAirportByIataBoundary7() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 7")
    void testCreateAirportBoundary7() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 7).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
