...
class ServiceEdgeCaseTest84 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 84")
    void testGetAirportByIataBoundary84() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 84")
    void testCreateAirportBoundary84() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 84).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
