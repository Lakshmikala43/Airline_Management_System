...
class ServiceEdgeCaseTest50 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 50")
    void testGetAirportByIataBoundary50() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 50")
    void testCreateAirportBoundary50() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 50).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
