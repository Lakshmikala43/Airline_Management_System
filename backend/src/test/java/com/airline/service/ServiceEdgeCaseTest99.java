...
class ServiceEdgeCaseTest99 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 99")
    void testGetAirportByIataBoundary99() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 99")
    void testCreateAirportBoundary99() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 99).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
