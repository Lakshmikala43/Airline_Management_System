...
class ServiceEdgeCaseTest30 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 30")
    void testGetAirportByIataBoundary30() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 30")
    void testCreateAirportBoundary30() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 30).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
