...
class ServiceEdgeCaseTest134 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 134")
    void testGetAirportByIataBoundary134() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 134")
    void testCreateAirportBoundary134() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 134).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
