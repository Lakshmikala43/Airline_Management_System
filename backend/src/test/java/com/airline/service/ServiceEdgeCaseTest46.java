...
class ServiceEdgeCaseTest46 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 46")
    void testGetAirportByIataBoundary46() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 46")
    void testCreateAirportBoundary46() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 46).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
