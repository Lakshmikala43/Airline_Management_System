...
class ServiceEdgeCaseTest106 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 106")
    void testGetAirportByIataBoundary106() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 106")
    void testCreateAirportBoundary106() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 106).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
