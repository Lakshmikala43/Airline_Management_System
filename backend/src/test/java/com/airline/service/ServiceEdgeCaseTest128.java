...
class ServiceEdgeCaseTest128 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 128")
    void testGetAirportByIataBoundary128() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 128")
    void testCreateAirportBoundary128() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 128).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
