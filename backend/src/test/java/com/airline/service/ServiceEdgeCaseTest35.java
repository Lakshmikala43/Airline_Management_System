...
class ServiceEdgeCaseTest35 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 35")
    void testGetAirportByIataBoundary35() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 35")
    void testCreateAirportBoundary35() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 35).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
