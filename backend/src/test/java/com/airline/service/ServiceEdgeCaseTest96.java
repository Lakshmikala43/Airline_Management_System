...
class ServiceEdgeCaseTest96 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 96")
    void testGetAirportByIataBoundary96() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 96")
    void testCreateAirportBoundary96() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 96).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
