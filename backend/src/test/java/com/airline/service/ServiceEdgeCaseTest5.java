...
class ServiceEdgeCaseTest5 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 5")
    void testGetAirportByIataBoundary5() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 5")
    void testCreateAirportBoundary5() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 5).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
