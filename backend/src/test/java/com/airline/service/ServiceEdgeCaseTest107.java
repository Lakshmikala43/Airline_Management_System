...
class ServiceEdgeCaseTest107 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 107")
    void testGetAirportByIataBoundary107() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 107")
    void testCreateAirportBoundary107() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 107).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
