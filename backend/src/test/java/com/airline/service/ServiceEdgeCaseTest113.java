...
class ServiceEdgeCaseTest113 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 113")
    void testGetAirportByIataBoundary113() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 113")
    void testCreateAirportBoundary113() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 113).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
