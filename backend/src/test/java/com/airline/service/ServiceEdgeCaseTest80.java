...
class ServiceEdgeCaseTest80 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 80")
    void testGetAirportByIataBoundary80() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 80")
    void testCreateAirportBoundary80() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 80).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
