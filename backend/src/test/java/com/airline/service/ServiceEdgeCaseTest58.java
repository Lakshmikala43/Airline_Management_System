...
class ServiceEdgeCaseTest58 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 58")
    void testGetAirportByIataBoundary58() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 58")
    void testCreateAirportBoundary58() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 58).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
