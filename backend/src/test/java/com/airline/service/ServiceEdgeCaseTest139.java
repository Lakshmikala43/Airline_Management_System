...
class ServiceEdgeCaseTest139 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 139")
    void testGetAirportByIataBoundary139() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 139")
    void testCreateAirportBoundary139() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 139).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
