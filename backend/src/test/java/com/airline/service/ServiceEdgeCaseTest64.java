...
class ServiceEdgeCaseTest64 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 64")
    void testGetAirportByIataBoundary64() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 64")
    void testCreateAirportBoundary64() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 64).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
