...
class ServiceEdgeCaseTest115 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 115")
    void testGetAirportByIataBoundary115() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 115")
    void testCreateAirportBoundary115() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 115).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
