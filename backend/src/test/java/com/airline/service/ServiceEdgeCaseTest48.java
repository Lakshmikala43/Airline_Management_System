...
class ServiceEdgeCaseTest48 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 48")
    void testGetAirportByIataBoundary48() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 48")
    void testCreateAirportBoundary48() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 48).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
