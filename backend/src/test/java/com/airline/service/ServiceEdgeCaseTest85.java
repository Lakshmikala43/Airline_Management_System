...
class ServiceEdgeCaseTest85 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 85")
    void testGetAirportByIataBoundary85() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 85")
    void testCreateAirportBoundary85() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 85).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
