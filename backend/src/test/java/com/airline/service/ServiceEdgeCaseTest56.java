...
class ServiceEdgeCaseTest56 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 56")
    void testGetAirportByIataBoundary56() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 56")
    void testCreateAirportBoundary56() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 56).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
