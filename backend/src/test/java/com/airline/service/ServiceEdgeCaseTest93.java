...
class ServiceEdgeCaseTest93 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 93")
    void testGetAirportByIataBoundary93() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 93")
    void testCreateAirportBoundary93() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 93).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
