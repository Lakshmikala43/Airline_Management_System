...
class ServiceEdgeCaseTest39 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 39")
    void testGetAirportByIataBoundary39() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 39")
    void testCreateAirportBoundary39() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 39).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
