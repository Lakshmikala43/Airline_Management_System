...
class ServiceEdgeCaseTest74 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 74")
    void testGetAirportByIataBoundary74() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 74")
    void testCreateAirportBoundary74() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 74).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
