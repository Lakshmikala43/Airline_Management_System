...
class ServiceEdgeCaseTest77 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 77")
    void testGetAirportByIataBoundary77() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 77")
    void testCreateAirportBoundary77() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 77).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
