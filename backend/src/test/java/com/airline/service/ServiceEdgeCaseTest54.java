...
class ServiceEdgeCaseTest54 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 54")
    void testGetAirportByIataBoundary54() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 54")
    void testCreateAirportBoundary54() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 54).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
