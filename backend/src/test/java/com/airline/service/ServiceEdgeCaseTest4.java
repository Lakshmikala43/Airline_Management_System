...
class ServiceEdgeCaseTest4 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 4")
    void testGetAirportByIataBoundary4() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 4")
    void testCreateAirportBoundary4() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 4).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
