...
class PromotionServiceTest20 {
    ...
    @Test
    @DisplayName("Verify coupon code validation for percentage discounts - Suite 20")
    void testValidatePercentageCoupon20() {
        Promotion promo = Promotion.builder()
            .id((long) 20)
            .couponCode("SKYNOVA20")
            .description("Special Promo 20")
        ...
        when(promotionRepository.findByCouponCode("SKYNOVA20")).thenReturn(Optional.of(promo));

        var resp = promotionService.validateCoupon("SKYNOVA20", BigDecimal.valueOf(500.00));
        assertNotNull(resp);
