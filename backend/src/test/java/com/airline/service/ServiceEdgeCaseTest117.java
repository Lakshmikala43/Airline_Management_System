...
class ServiceEdgeCaseTest117 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 117")
    void testGetAirportByIataBoundary117() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 117")
    void testCreateAirportBoundary117() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 117).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
