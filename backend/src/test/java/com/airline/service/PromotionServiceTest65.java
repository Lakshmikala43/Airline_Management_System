...
class PromotionServiceTest65 {
    ...
    @Test
    @DisplayName("Verify coupon code validation for percentage discounts - Suite 65")
    void testValidatePercentageCoupon65() {
        Promotion promo = Promotion.builder()
            .id((long) 65)
            .couponCode("SKYNOVA65")
            .description("Special Promo 65")
        ...
        when(promotionRepository.findByCouponCode("SKYNOVA65")).thenReturn(Optional.of(promo));

        var resp = promotionService.validateCoupon("SKYNOVA65", BigDecimal.valueOf(500.00));
        assertNotNull(resp);
