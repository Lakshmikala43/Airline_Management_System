...
class ServiceEdgeCaseTest27 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 27")
    void testGetAirportByIataBoundary27() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 27")
    void testCreateAirportBoundary27() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 27).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
