...
class ServiceEdgeCaseTest147 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 147")
    void testGetAirportByIataBoundary147() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 147")
    void testCreateAirportBoundary147() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 147).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
