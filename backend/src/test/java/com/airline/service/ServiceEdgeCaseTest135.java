...
class ServiceEdgeCaseTest135 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 135")
    void testGetAirportByIataBoundary135() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 135")
    void testCreateAirportBoundary135() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 135).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
