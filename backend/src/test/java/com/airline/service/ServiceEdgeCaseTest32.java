...
class ServiceEdgeCaseTest32 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 32")
    void testGetAirportByIataBoundary32() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 32")
    void testCreateAirportBoundary32() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 32).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
