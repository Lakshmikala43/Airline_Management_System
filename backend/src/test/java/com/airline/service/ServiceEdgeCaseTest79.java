...
class ServiceEdgeCaseTest79 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 79")
    void testGetAirportByIataBoundary79() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 79")
    void testCreateAirportBoundary79() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 79).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
