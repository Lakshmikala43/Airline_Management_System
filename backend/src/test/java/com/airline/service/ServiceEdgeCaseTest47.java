...
class ServiceEdgeCaseTest47 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 47")
    void testGetAirportByIataBoundary47() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 47")
    void testCreateAirportBoundary47() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 47).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
