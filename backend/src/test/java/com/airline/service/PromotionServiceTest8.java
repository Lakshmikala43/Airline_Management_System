...
class PromotionServiceTest8 {
    ...
    @Test
    @DisplayName("Verify coupon code validation for percentage discounts - Suite 8")
    void testValidatePercentageCoupon8() {
        Promotion promo = Promotion.builder()
            .id((long) 8)
            .couponCode("SKYNOVA8")
            .description("Special Promo 8")
        ...
        when(promotionRepository.findByCouponCode("SKYNOVA8")).thenReturn(Optional.of(promo));

        var resp = promotionService.validateCoupon("SKYNOVA8", BigDecimal.valueOf(500.00));
        assertNotNull(resp);
