...
class ServiceEdgeCaseTest91 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 91")
    void testGetAirportByIataBoundary91() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 91")
    void testCreateAirportBoundary91() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 91).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
