...
class ServiceEdgeCaseTest120 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 120")
    void testGetAirportByIataBoundary120() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 120")
    void testCreateAirportBoundary120() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 120).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
