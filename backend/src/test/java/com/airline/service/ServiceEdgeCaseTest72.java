...
class ServiceEdgeCaseTest72 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 72")
    void testGetAirportByIataBoundary72() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 72")
    void testCreateAirportBoundary72() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 72).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
