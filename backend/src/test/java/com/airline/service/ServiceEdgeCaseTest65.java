...
class ServiceEdgeCaseTest65 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 65")
    void testGetAirportByIataBoundary65() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 65")
    void testCreateAirportBoundary65() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 65).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
