...
class PromotionServiceTest5 {
    ...
    @Test
    @DisplayName("Verify coupon code validation for percentage discounts - Suite 5")
    void testValidatePercentageCoupon5() {
        Promotion promo = Promotion.builder()
            .id((long) 5)
            .couponCode("SKYNOVA5")
            .description("Special Promo 5")
        ...
        when(promotionRepository.findByCouponCode("SKYNOVA5")).thenReturn(Optional.of(promo));

        var resp = promotionService.validateCoupon("SKYNOVA5", BigDecimal.valueOf(500.00));
        assertNotNull(resp);
