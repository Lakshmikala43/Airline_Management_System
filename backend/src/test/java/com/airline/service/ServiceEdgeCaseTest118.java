...
class ServiceEdgeCaseTest118 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 118")
    void testGetAirportByIataBoundary118() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 118")
    void testCreateAirportBoundary118() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 118).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
