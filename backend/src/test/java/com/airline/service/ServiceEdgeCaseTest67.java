...
class ServiceEdgeCaseTest67 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 67")
    void testGetAirportByIataBoundary67() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 67")
    void testCreateAirportBoundary67() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 67).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
