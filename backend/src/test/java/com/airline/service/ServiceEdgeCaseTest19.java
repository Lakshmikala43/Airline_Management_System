...
class ServiceEdgeCaseTest19 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 19")
    void testGetAirportByIataBoundary19() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 19")
    void testCreateAirportBoundary19() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 19).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
