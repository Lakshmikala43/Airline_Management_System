...
class ServiceEdgeCaseTest73 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 73")
    void testGetAirportByIataBoundary73() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 73")
    void testCreateAirportBoundary73() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 73).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
