...
class ServiceEdgeCaseTest49 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 49")
    void testGetAirportByIataBoundary49() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 49")
    void testCreateAirportBoundary49() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 49).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
