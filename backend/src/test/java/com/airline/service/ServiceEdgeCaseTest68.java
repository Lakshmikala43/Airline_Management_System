...
class ServiceEdgeCaseTest68 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 68")
    void testGetAirportByIataBoundary68() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 68")
    void testCreateAirportBoundary68() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 68).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
