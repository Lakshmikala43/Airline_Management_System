...
class ServiceEdgeCaseTest62 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 62")
    void testGetAirportByIataBoundary62() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 62")
    void testCreateAirportBoundary62() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 62).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
