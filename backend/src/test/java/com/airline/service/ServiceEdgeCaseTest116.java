...
class ServiceEdgeCaseTest116 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 116")
    void testGetAirportByIataBoundary116() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 116")
    void testCreateAirportBoundary116() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 116).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
