...
class ServiceEdgeCaseTest34 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 34")
    void testGetAirportByIataBoundary34() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 34")
    void testCreateAirportBoundary34() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 34).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
