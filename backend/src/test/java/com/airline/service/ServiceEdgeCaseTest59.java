...
class ServiceEdgeCaseTest59 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 59")
    void testGetAirportByIataBoundary59() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 59")
    void testCreateAirportBoundary59() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 59).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
