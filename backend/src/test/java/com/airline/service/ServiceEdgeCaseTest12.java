...
class ServiceEdgeCaseTest12 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 12")
    void testGetAirportByIataBoundary12() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 12")
    void testCreateAirportBoundary12() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 12).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
