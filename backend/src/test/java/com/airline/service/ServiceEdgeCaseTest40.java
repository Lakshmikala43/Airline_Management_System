...
class ServiceEdgeCaseTest40 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 40")
    void testGetAirportByIataBoundary40() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 40")
    void testCreateAirportBoundary40() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 40).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
