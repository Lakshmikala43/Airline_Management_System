...
class ServiceEdgeCaseTest122 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 122")
    void testGetAirportByIataBoundary122() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 122")
    void testCreateAirportBoundary122() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 122).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
