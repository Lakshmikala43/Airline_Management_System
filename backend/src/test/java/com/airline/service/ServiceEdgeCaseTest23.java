...
class ServiceEdgeCaseTest23 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 23")
    void testGetAirportByIataBoundary23() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 23")
    void testCreateAirportBoundary23() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 23).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
