...
class ServiceEdgeCaseTest133 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 133")
    void testGetAirportByIataBoundary133() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 133")
    void testCreateAirportBoundary133() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 133).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
