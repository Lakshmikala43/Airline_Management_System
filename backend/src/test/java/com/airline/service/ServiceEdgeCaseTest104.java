...
class ServiceEdgeCaseTest104 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 104")
    void testGetAirportByIataBoundary104() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 104")
    void testCreateAirportBoundary104() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 104).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
