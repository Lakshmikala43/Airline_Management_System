...
class ServiceEdgeCaseTest26 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 26")
    void testGetAirportByIataBoundary26() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 26")
    void testCreateAirportBoundary26() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 26).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
