...
class PromotionServiceTest36 {
    ...
    @Test
    @DisplayName("Verify coupon code validation for percentage discounts - Suite 36")
    void testValidatePercentageCoupon36() {
        Promotion promo = Promotion.builder()
            .id((long) 36)
            .couponCode("SKYNOVA36")
            .description("Special Promo 36")
        ...
        when(promotionRepository.findByCouponCode("SKYNOVA36")).thenReturn(Optional.of(promo));

        var resp = promotionService.validateCoupon("SKYNOVA36", BigDecimal.valueOf(500.00));
        assertNotNull(resp);
