...
class ServiceEdgeCaseTest53 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 53")
    void testGetAirportByIataBoundary53() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 53")
    void testCreateAirportBoundary53() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 53).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
