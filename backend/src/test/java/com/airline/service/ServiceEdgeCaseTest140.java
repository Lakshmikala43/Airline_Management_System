...
class ServiceEdgeCaseTest140 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 140")
    void testGetAirportByIataBoundary140() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 140")
    void testCreateAirportBoundary140() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 140).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
