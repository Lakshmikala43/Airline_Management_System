...
class ServiceEdgeCaseTest52 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 52")
    void testGetAirportByIataBoundary52() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 52")
    void testCreateAirportBoundary52() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 52).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
