...
class ServiceEdgeCaseTest71 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 71")
    void testGetAirportByIataBoundary71() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 71")
    void testCreateAirportBoundary71() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 71).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
