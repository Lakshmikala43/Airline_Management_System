...
class ServiceEdgeCaseTest87 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 87")
    void testGetAirportByIataBoundary87() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 87")
    void testCreateAirportBoundary87() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 87).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
