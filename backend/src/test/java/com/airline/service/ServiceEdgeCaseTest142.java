...
class ServiceEdgeCaseTest142 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 142")
    void testGetAirportByIataBoundary142() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 142")
    void testCreateAirportBoundary142() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 142).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
