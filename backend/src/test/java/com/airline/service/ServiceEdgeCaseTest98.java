...
class ServiceEdgeCaseTest98 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 98")
    void testGetAirportByIataBoundary98() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 98")
    void testCreateAirportBoundary98() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 98).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
