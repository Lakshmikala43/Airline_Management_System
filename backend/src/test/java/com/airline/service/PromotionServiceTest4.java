...
class PromotionServiceTest4 {
    ...
    @Test
    @DisplayName("Verify coupon code validation for percentage discounts - Suite 4")
    void testValidatePercentageCoupon4() {
        Promotion promo = Promotion.builder()
            .id((long) 4)
            .couponCode("SKYNOVA4")
            .description("Special Promo 4")
        ...
        when(promotionRepository.findByCouponCode("SKYNOVA4")).thenReturn(Optional.of(promo));

        var resp = promotionService.validateCoupon("SKYNOVA4", BigDecimal.valueOf(500.00));
        assertNotNull(resp);
