...
class ServiceEdgeCaseTest9 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 9")
    void testGetAirportByIataBoundary9() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 9")
    void testCreateAirportBoundary9() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 9).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
