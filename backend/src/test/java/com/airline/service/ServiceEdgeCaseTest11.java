...
class ServiceEdgeCaseTest11 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 11")
    void testGetAirportByIataBoundary11() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 11")
    void testCreateAirportBoundary11() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 11).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
