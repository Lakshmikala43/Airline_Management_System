...
class ServiceEdgeCaseTest28 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 28")
    void testGetAirportByIataBoundary28() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 28")
    void testCreateAirportBoundary28() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 28).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
