...
class ServiceEdgeCaseTest25 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 25")
    void testGetAirportByIataBoundary25() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 25")
    void testCreateAirportBoundary25() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 25).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
