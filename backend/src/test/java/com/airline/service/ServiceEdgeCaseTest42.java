...
class ServiceEdgeCaseTest42 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 42")
    void testGetAirportByIataBoundary42() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 42")
    void testCreateAirportBoundary42() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 42).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
