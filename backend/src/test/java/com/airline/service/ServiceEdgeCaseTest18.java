...
class ServiceEdgeCaseTest18 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 18")
    void testGetAirportByIataBoundary18() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 18")
    void testCreateAirportBoundary18() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 18).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
