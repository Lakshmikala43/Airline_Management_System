...
class ServiceEdgeCaseTest82 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 82")
    void testGetAirportByIataBoundary82() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 82")
    void testCreateAirportBoundary82() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 82).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
