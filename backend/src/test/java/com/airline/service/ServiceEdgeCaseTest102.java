...
class ServiceEdgeCaseTest102 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 102")
    void testGetAirportByIataBoundary102() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 102")
    void testCreateAirportBoundary102() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 102).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
