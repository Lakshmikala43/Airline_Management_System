...
class ServiceEdgeCaseTest81 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 81")
    void testGetAirportByIataBoundary81() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 81")
    void testCreateAirportBoundary81() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 81).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
