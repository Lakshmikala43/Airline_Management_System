...
class ServiceEdgeCaseTest136 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 136")
    void testGetAirportByIataBoundary136() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 136")
    void testCreateAirportBoundary136() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 136).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
