...
class ServiceEdgeCaseTest61 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 61")
    void testGetAirportByIataBoundary61() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 61")
    void testCreateAirportBoundary61() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 61).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
