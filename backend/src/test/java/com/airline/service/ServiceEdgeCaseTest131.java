...
class ServiceEdgeCaseTest131 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 131")
    void testGetAirportByIataBoundary131() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 131")
    void testCreateAirportBoundary131() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 131).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
