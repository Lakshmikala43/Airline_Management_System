...
class ServiceEdgeCaseTest88 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 88")
    void testGetAirportByIataBoundary88() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 88")
    void testCreateAirportBoundary88() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 88).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
