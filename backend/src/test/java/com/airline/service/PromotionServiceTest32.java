...
class PromotionServiceTest32 {
    ...
    @Test
    @DisplayName("Verify coupon code validation for percentage discounts - Suite 32")
    void testValidatePercentageCoupon32() {
        Promotion promo = Promotion.builder()
            .id((long) 32)
            .couponCode("SKYNOVA32")
            .description("Special Promo 32")
        ...
        when(promotionRepository.findByCouponCode("SKYNOVA32")).thenReturn(Optional.of(promo));

        var resp = promotionService.validateCoupon("SKYNOVA32", BigDecimal.valueOf(500.00));
        assertNotNull(resp);
