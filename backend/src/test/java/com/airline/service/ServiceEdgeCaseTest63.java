...
class ServiceEdgeCaseTest63 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 63")
    void testGetAirportByIataBoundary63() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 63")
    void testCreateAirportBoundary63() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 63).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
