...
class ServiceEdgeCaseTest29 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 29")
    void testGetAirportByIataBoundary29() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 29")
    void testCreateAirportBoundary29() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 29).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
