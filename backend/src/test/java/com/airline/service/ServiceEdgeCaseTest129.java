...
class ServiceEdgeCaseTest129 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 129")
    void testGetAirportByIataBoundary129() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 129")
    void testCreateAirportBoundary129() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 129).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
