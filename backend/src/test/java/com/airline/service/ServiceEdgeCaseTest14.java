...
class ServiceEdgeCaseTest14 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 14")
    void testGetAirportByIataBoundary14() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 14")
    void testCreateAirportBoundary14() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 14).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
