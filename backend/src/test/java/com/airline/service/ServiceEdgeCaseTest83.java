...
class ServiceEdgeCaseTest83 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 83")
    void testGetAirportByIataBoundary83() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 83")
    void testCreateAirportBoundary83() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 83).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
