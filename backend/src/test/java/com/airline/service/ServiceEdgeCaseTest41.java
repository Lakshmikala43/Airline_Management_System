...
class ServiceEdgeCaseTest41 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 41")
    void testGetAirportByIataBoundary41() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 41")
    void testCreateAirportBoundary41() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 41).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
