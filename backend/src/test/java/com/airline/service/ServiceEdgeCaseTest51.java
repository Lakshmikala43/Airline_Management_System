...
class ServiceEdgeCaseTest51 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 51")
    void testGetAirportByIataBoundary51() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 51")
    void testCreateAirportBoundary51() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 51).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
