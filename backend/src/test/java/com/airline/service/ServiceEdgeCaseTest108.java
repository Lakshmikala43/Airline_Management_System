...
class ServiceEdgeCaseTest108 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 108")
    void testGetAirportByIataBoundary108() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 108")
    void testCreateAirportBoundary108() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 108).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
