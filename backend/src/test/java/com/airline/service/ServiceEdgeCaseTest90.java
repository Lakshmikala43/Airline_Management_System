...
class ServiceEdgeCaseTest90 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 90")
    void testGetAirportByIataBoundary90() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 90")
    void testCreateAirportBoundary90() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 90).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
