...
class ServiceEdgeCaseTest144 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 144")
    void testGetAirportByIataBoundary144() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 144")
    void testCreateAirportBoundary144() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 144).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
