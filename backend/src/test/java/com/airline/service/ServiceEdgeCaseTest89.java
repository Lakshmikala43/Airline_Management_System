...
class ServiceEdgeCaseTest89 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 89")
    void testGetAirportByIataBoundary89() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 89")
    void testCreateAirportBoundary89() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 89).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
