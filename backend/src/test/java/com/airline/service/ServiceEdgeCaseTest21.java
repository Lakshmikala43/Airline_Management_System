...
class ServiceEdgeCaseTest21 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 21")
    void testGetAirportByIataBoundary21() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 21")
    void testCreateAirportBoundary21() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 21).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
