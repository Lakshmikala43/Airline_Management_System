...
class ServiceEdgeCaseTest100 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 100")
    void testGetAirportByIataBoundary100() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 100")
    void testCreateAirportBoundary100() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 100).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
