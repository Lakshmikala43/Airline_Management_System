...
class ServiceEdgeCaseTest132 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 132")
    void testGetAirportByIataBoundary132() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 132")
    void testCreateAirportBoundary132() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 132).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
