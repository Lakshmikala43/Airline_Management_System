...
class ServiceEdgeCaseTest20 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 20")
    void testGetAirportByIataBoundary20() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 20")
    void testCreateAirportBoundary20() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 20).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
