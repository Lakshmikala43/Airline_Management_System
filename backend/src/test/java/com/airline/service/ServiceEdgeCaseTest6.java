...
class ServiceEdgeCaseTest6 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 6")
    void testGetAirportByIataBoundary6() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 6")
    void testCreateAirportBoundary6() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 6).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
