...
class ServiceEdgeCaseTest127 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 127")
    void testGetAirportByIataBoundary127() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 127")
    void testCreateAirportBoundary127() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 127).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
