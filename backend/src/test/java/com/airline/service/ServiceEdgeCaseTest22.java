...
class ServiceEdgeCaseTest22 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 22")
    void testGetAirportByIataBoundary22() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 22")
    void testCreateAirportBoundary22() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 22).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
