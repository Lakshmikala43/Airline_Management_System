...
class ServiceEdgeCaseTest17 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 17")
    void testGetAirportByIataBoundary17() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 17")
    void testCreateAirportBoundary17() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 17).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
