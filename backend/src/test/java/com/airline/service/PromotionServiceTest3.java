...
class PromotionServiceTest3 {
    ...
    @Test
    @DisplayName("Verify coupon code validation for percentage discounts - Suite 3")
    void testValidatePercentageCoupon3() {
        Promotion promo = Promotion.builder()
            .id((long) 3)
            .couponCode("SKYNOVA3")
            .description("Special Promo 3")
        ...
        when(promotionRepository.findByCouponCode("SKYNOVA3")).thenReturn(Optional.of(promo));

        var resp = promotionService.validateCoupon("SKYNOVA3", BigDecimal.valueOf(500.00));
        assertNotNull(resp);
