...
class ServiceEdgeCaseTest145 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 145")
    void testGetAirportByIataBoundary145() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 145")
    void testCreateAirportBoundary145() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 145).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
