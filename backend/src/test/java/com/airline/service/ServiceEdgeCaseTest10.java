...
class ServiceEdgeCaseTest10 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 10")
    void testGetAirportByIataBoundary10() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 10")
    void testCreateAirportBoundary10() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 10).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
