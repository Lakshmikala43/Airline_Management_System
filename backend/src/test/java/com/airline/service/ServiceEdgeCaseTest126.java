...
class ServiceEdgeCaseTest126 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 126")
    void testGetAirportByIataBoundary126() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 126")
    void testCreateAirportBoundary126() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 126).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
