...
class ServiceEdgeCaseTest101 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 101")
    void testGetAirportByIataBoundary101() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 101")
    void testCreateAirportBoundary101() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 101).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
