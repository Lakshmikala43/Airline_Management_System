...
class PromotionServiceTest64 {
    ...
    @Test
    @DisplayName("Verify coupon code validation for percentage discounts - Suite 64")
    void testValidatePercentageCoupon64() {
        Promotion promo = Promotion.builder()
            .id((long) 64)
            .couponCode("SKYNOVA64")
            .description("Special Promo 64")
        ...
        when(promotionRepository.findByCouponCode("SKYNOVA64")).thenReturn(Optional.of(promo));

        var resp = promotionService.validateCoupon("SKYNOVA64", BigDecimal.valueOf(500.00));
        assertNotNull(resp);
