...
class ServiceEdgeCaseTest36 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 36")
    void testGetAirportByIataBoundary36() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 36")
    void testCreateAirportBoundary36() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 36).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
