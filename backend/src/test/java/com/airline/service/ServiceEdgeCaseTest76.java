...
class ServiceEdgeCaseTest76 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 76")
    void testGetAirportByIataBoundary76() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 76")
    void testCreateAirportBoundary76() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 76).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
