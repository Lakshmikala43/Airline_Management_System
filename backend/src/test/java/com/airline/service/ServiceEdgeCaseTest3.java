...
class ServiceEdgeCaseTest3 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 3")
    void testGetAirportByIataBoundary3() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 3")
    void testCreateAirportBoundary3() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 3).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
