...
class ServiceEdgeCaseTest148 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 148")
    void testGetAirportByIataBoundary148() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 148")
    void testCreateAirportBoundary148() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 148).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
