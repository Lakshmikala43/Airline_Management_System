...
class ServiceEdgeCaseTest143 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 143")
    void testGetAirportByIataBoundary143() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 143")
    void testCreateAirportBoundary143() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 143).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
