...
class ServiceEdgeCaseTest119 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 119")
    void testGetAirportByIataBoundary119() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 119")
    void testCreateAirportBoundary119() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 119).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
