...
class ServiceEdgeCaseTest33 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 33")
    void testGetAirportByIataBoundary33() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 33")
    void testCreateAirportBoundary33() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 33).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
