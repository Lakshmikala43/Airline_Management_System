...
class ServiceEdgeCaseTest16 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 16")
    void testGetAirportByIataBoundary16() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 16")
    void testCreateAirportBoundary16() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 16).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
