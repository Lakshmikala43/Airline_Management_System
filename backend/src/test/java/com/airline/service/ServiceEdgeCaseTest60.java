...
class ServiceEdgeCaseTest60 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 60")
    void testGetAirportByIataBoundary60() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 60")
    void testCreateAirportBoundary60() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 60).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
