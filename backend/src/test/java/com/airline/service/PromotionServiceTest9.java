...
class PromotionServiceTest9 {
    ...
    @Test
    @DisplayName("Verify coupon code validation for percentage discounts - Suite 9")
    void testValidatePercentageCoupon9() {
        Promotion promo = Promotion.builder()
            .id((long) 9)
            .couponCode("SKYNOVA9")
            .description("Special Promo 9")
        ...
        when(promotionRepository.findByCouponCode("SKYNOVA9")).thenReturn(Optional.of(promo));

        var resp = promotionService.validateCoupon("SKYNOVA9", BigDecimal.valueOf(500.00));
        assertNotNull(resp);
