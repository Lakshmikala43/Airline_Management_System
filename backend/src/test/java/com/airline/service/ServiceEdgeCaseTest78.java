...
class ServiceEdgeCaseTest78 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 78")
    void testGetAirportByIataBoundary78() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 78")
    void testCreateAirportBoundary78() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 78).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
