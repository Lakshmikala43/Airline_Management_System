...
class ServiceEdgeCaseTest38 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 38")
    void testGetAirportByIataBoundary38() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 38")
    void testCreateAirportBoundary38() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 38).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
