...
class ServiceEdgeCaseTest45 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 45")
    void testGetAirportByIataBoundary45() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 45")
    void testCreateAirportBoundary45() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 45).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
