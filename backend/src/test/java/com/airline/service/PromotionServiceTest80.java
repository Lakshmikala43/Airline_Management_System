...
class PromotionServiceTest80 {
    ...
    @Test
    @DisplayName("Verify coupon code validation for percentage discounts - Suite 80")
    void testValidatePercentageCoupon80() {
        Promotion promo = Promotion.builder()
            .id((long) 80)
            .couponCode("SKYNOVA80")
            .description("Special Promo 80")
        ...
        when(promotionRepository.findByCouponCode("SKYNOVA80")).thenReturn(Optional.of(promo));

        var resp = promotionService.validateCoupon("SKYNOVA80", BigDecimal.valueOf(500.00));
        assertNotNull(resp);
