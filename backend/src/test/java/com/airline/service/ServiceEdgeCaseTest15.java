...
class ServiceEdgeCaseTest15 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 15")
    void testGetAirportByIataBoundary15() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 15")
    void testCreateAirportBoundary15() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 15).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
