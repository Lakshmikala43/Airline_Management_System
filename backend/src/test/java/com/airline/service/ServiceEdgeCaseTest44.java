...
class ServiceEdgeCaseTest44 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 44")
    void testGetAirportByIataBoundary44() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 44")
    void testCreateAirportBoundary44() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 44).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
