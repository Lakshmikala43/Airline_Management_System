...
class ServiceEdgeCaseTest130 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 130")
    void testGetAirportByIataBoundary130() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 130")
    void testCreateAirportBoundary130() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 130).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
