...
class ServiceEdgeCaseTest114 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 114")
    void testGetAirportByIataBoundary114() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 114")
    void testCreateAirportBoundary114() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 114).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
