...
class ServiceEdgeCaseTest146 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 146")
    void testGetAirportByIataBoundary146() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 146")
    void testCreateAirportBoundary146() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 146).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
