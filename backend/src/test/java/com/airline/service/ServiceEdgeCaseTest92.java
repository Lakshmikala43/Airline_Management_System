...
class ServiceEdgeCaseTest92 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 92")
    void testGetAirportByIataBoundary92() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 92")
    void testCreateAirportBoundary92() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 92).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
