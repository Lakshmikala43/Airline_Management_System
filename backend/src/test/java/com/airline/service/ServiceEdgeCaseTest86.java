...
class ServiceEdgeCaseTest86 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 86")
    void testGetAirportByIataBoundary86() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 86")
    void testCreateAirportBoundary86() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 86).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
