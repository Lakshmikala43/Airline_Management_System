...
class ServiceEdgeCaseTest75 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 75")
    void testGetAirportByIataBoundary75() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 75")
    void testCreateAirportBoundary75() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 75).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
