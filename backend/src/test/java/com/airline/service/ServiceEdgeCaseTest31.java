...
class ServiceEdgeCaseTest31 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 31")
    void testGetAirportByIataBoundary31() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 31")
    void testCreateAirportBoundary31() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 31).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
