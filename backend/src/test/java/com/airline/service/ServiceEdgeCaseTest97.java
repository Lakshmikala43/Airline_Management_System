...
class ServiceEdgeCaseTest97 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 97")
    void testGetAirportByIataBoundary97() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 97")
    void testCreateAirportBoundary97() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 97).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
