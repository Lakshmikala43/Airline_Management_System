...
class ServiceEdgeCaseTest66 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 66")
    void testGetAirportByIataBoundary66() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 66")
    void testCreateAirportBoundary66() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 66).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
