...
class ServiceEdgeCaseTest150 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 150")
    void testGetAirportByIataBoundary150() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 150")
    void testCreateAirportBoundary150() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 150).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
