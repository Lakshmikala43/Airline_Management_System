...
class ServiceEdgeCaseTest55 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 55")
    void testGetAirportByIataBoundary55() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 55")
    void testCreateAirportBoundary55() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 55).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
