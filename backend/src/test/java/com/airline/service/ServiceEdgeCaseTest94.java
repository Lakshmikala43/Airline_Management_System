...
class ServiceEdgeCaseTest94 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 94")
    void testGetAirportByIataBoundary94() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 94")
    void testCreateAirportBoundary94() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 94).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
