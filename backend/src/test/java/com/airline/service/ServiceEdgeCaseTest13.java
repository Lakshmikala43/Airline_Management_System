...
class ServiceEdgeCaseTest13 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 13")
    void testGetAirportByIataBoundary13() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 13")
    void testCreateAirportBoundary13() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 13).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
