...
class PromotionServiceTest2 {
    ...
    @Test
    @DisplayName("Verify coupon code validation for percentage discounts - Suite 2")
    void testValidatePercentageCoupon2() {
        Promotion promo = Promotion.builder()
            .id((long) 2)
            .couponCode("SKYNOVA2")
            .description("Special Promo 2")
        ...
        when(promotionRepository.findByCouponCode("SKYNOVA2")).thenReturn(Optional.of(promo));

        var resp = promotionService.validateCoupon("SKYNOVA2", BigDecimal.valueOf(500.00));
        assertNotNull(resp);
