...
class ServiceEdgeCaseTest121 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 121")
    void testGetAirportByIataBoundary121() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 121")
    void testCreateAirportBoundary121() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 121).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
