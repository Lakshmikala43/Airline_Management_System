...
class ServiceEdgeCaseTest137 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 137")
    void testGetAirportByIataBoundary137() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 137")
    void testCreateAirportBoundary137() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 137).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
