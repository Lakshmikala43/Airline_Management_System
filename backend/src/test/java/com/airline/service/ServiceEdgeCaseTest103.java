...
class ServiceEdgeCaseTest103 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 103")
    void testGetAirportByIataBoundary103() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 103")
    void testCreateAirportBoundary103() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 103).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
