...
class ServiceEdgeCaseTest8 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 8")
    void testGetAirportByIataBoundary8() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 8")
    void testCreateAirportBoundary8() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 8).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
