...
class ServiceEdgeCaseTest149 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 149")
    void testGetAirportByIataBoundary149() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 149")
    void testCreateAirportBoundary149() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 149).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
