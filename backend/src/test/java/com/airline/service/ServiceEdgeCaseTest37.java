...
class ServiceEdgeCaseTest37 {
    ...
    @Test
    @DisplayName("Verify airport IATA lookup boundary conditions - Edge Suite 37")
    void testGetAirportByIataBoundary37() {
    ...
    @Test
    @DisplayName("Verify airport creation boundary validation - Edge Suite 37")
    void testCreateAirportBoundary37() {
        when(airportRepository.existsByIataCode("SFO")).thenReturn(false);
        Airport airport = Airport.builder().id((long) 37).iataCode("SFO").name("San Francisco Intl").city("SF").country("US").timeZone("PST").build();
